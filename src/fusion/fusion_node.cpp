...
class FusionNode : public rclcpp::Node {
 ...
 private:
  ...
  void initializeEKF() {
    ...
    lidar_noise_ =
        Eigen::Matrix<double, LIDAR_MEAS_DIM, LIDAR_MEAS_DIM>::Zero();
  ...
  void imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg) {
    ...
    // az_compensated = msg->linear_acceleration.z − gravity_;          //
    // unused in 2-D
    ...
    covariance_ =
        IKH * covariance_ * IKH.transpose() + K * imu_noise_ * K.transpose();
  }
  ...
  void lidarCallback(const sensor_msgs::msg::LaserScan::SharedPtr msg) {
    ...
    if (valid == 0) {
      RCLCPP_WARN(this->get_logger(),
                  "lidarCallback: no valid readings — skipping");
      return;
    }
    ...
    const StateMatrix IKH = StateMatrix::Identity() - K * H;
    covariance_ =
        IKH * covariance_ * IKH.transpose() + K * lidar_noise_ * K.transpose();
  }

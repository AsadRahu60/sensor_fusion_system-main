...
class SafetyMonitorNode : public rclcpp::Node {
 public:
  SafetyMonitorNode()
      : Node("safety_monitor_node"), pose_healthy_(true), imu_healthy_(true) {
    ...
    watchdog_timer_ = this->create_wall_timer(
        std::chrono::milliseconds(500), [this]() { this->checkDropouts(); });
    ...
    RCLCPP_INFO(this->get_logger(),
                "Safety Monitor initialized (dropout timeout = %.1f s)",
                kDropoutTimeout);
  }
 ...
 private:
  ...
  void checkPoseSafety(
    ...
    if (pos_cov > kMaxCovariance) {
      RCLCPP_WARN(
          this->get_logger(),
          "Covariance too large (%.3f) — Kalman filter may be diverging!",
          pos_cov);
      ok = false;
    }
  ...
  void checkDropouts() {
    ...
    if (pose_age > kDropoutTimeout) {
      if (pose_healthy_) {
        RCLCPP_ERROR(this->get_logger(),
                     "Fusion pose dropout detected! No message for %.1f s",
                     pose_age);
      }
      pose_healthy_ = false;
    }
  ...
  bool isSystemHealthy() const { return pose_healthy_ && imu_healthy_; }
  ...
  // access is safe.  For MultiThreadedExecutor, wrap flags in
  // std::atomic<bool>.
};

...
static void ekfPredict(Vec& x, Mat& P, const Eigen::Vector2d& u, double dt,
                       const Mat& Q) {
...
template <int M>
static void kalmanUpdate(Vec& x, Mat& P, const Eigen::Matrix<double, M, N>& H,
...
TEST(EKFTest, CovarianceRemainsPositiveSemiDefinite) {
  ...
  const Eigen::Matrix<double, 1, 1> meas_gyro =
